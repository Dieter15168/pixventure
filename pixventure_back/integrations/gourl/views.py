...
def callback(request, *args, **kwargs):
    ...
    Behavior:
      ...
      3. Logs the callback in PaymentCallbackLog with:
            - status: 0 for "payment_received", 1 for "payment_received_unrecognised".
            - confirmed: True if confirmed=="1", else False.
            - raw_data: The raw callback data.
      4. Processes the callback:
         - For unconfirmed payment (confirmed == "0" and status == "payment_received"):
             * If the received amount matches the transaction amount, update the Transaction’s metadata
               and mark it as successful (set status to STATUS_COMPLETED).
         - For confirmed payment (confirmed == "1"):
             * Only update the metadata for logging purposes without changing the Transaction’s status.
      5. Returns the appropriate response string required by the GoURL system.
    """
    ...
    if confirmed == "0" and status_field == "payment_received":
        if transaction.amount == amountusd:
            ...
            # Update the transaction using its ID.
            payment_manager.update_transaction(transaction.id, target_status=Transaction.STATUS_COMPLETED, metadata_updates=metadata)
            return HttpResponse("cryptobox_newrecord")
        else:
            return HttpResponse("cryptobox_nochanges")
    
    # Process confirmed payment: log additional metadata without changing status.
    elif confirmed == "1":
        metadata = {
        ...
        payment_manager.update_transaction(transaction.id, target_status=None, metadata_updates=metadata)
